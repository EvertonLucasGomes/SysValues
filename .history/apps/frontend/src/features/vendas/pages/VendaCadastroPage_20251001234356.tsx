...
import { UapSelect } from "@/components/UapSelect";
...
export default function VendaCadastroPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.uapId || !formData.produto || !formData.quantidade || !formData.precoUnitario) {
        ...
        <div className="card p-8 border-agro-200">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <UapSelect value={formData.uapId} onChange={(v) => setField("uapId", v)} uaps={uaps} />
              <ProductSelect value={formData.produto} onChange={(v) => setField("produto", v)} products={products} />
            </div>
